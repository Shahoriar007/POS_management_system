...
include 'config.php';
$sessionEmail = $_SESSION['operator_email'];
$sql = "SELECT * FROM operators WHERE operator_email='$sessionEmail'";
$result = mysqli_query($conn, $sql);

if ($result->num_rows > 0) {
  $row = mysqli_fetch_assoc($result);
  $userName = $row['operator_name'];
  $userEmail = $row['operator_email'];
  $phoneNo = $row['operator_phone'];
  $userAddress = $row['operator_address'];
}
?>
<!DOCTYPE html>
<html lang="en">
...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    <ul class="navbar-nav mr-auto">
      ...
      <li class="nav-item">
        <a class="nav-link" href="order_invoice.php">Order</a>
      </li>
      <li class="nav-item">
